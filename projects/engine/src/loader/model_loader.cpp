...
BasicMaterial Loader::LoadMaterialTextures( const aiMaterial* mat, const path& path )
{
	...
	for ( u32 i = 0; i < 3; i++ )
	{
		for ( u32 i = 0; i < 3; i++ )
		{
			for ( u32 j = 0; j < mat->GetTextureCount( types[i] ); j++ )
			{
				aiString str;
	...
	//Ref<Texture2D> white_plug = LoadTexture2DSingleColor( "white plug", vec4( 1.0f ) );
